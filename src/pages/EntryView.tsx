...
import { Copy, Check, Download, Filter, FileText, Layers, X, ZoomIn, ZoomOut, RotateCcw, Loader2 } from "lucide-react";
...
function CopyCell({ value, mono, id }: { value: string; mono?: boolean; id: string }) {
  ...
  return (
    <button onClick={copy} className={`group flex items-center gap-1 text-sm text-left ${mono ? "vault-mono" : ""} text-foreground hover:text-accent transition-colors`}>
...
export default function EntryView() {
  ...
  return (
    <div className="space-y-4 max-w-[1600px] relative">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Copy fields into AppFolio, mark as recorded, then create deposit batches.</p>
        </div>
      </div>
                          ...
                          {r.memo ? (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <button onClick={() => { navigator.clipboard.writeText(r.memo || ""); toast({ title: "Memo copied" }); }} className="group flex items-center gap-1 text-sm text-foreground hover:text-accent transition-colors max-w-[140px]">
                              ...
                              <TooltipContent side="top" className="max-w-xs"><p className="text-xs whitespace-pre-wrap">{r.memo}</p></TooltipContent>
                            </Tooltip>
