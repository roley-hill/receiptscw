...
import { mockReceipts, properties, paymentTypes } from "@/lib/mockData";
...
export default function SearchPage() {
  ...
  const results = mockReceipts.filter((r) => {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Find receipts by tenant, property, amount, reference, or text.</p>
      </div>
      ...
      <div className="vault-card p-4 flex items-center gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by tenant, property, amount, check #, memo..."
            className="w-full h-10 rounded-md border border-input bg-background pl-10 pr-10 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          />
          {query && (
          ...
          <tbody>
            {results.map((r) => (
              <tr key={r.id} className="vault-table-row">
                <td className="px-4 py-3 text-xs vault-mono text-vault-blue cursor-pointer hover:underline">{r.id}</td>
                ...
                <td className="px-4 py-3 text-sm vault-mono text-muted-foreground">{r.reference}</td>
