...
// ── Memory safety: cap each file at 512 KB before base64-encoding ─────────────
...
/** Read up to MAX_FILE_BYTES from a File and return base64 + mime */
...
// ── Phase 1: detect address from a SINGLE file ────────────────────────────────
...
async function detectAddress(file: File): Promise<{
  ...
  const contentParts: unknown[] = [
    {
      type: "text",
      text: `You are a real-estate document analyst. Examine this document (first page only) and extract the PROPERTY building address (NOT a tenant's mailing address).
Return ONLY valid JSON, no markdown fences:
{ "address": "full street address", "city": "city", "state": "2-letter state", "postal_code": "5-digit zip", "confidence": 0.0-1.0 }
If you cannot determine it, set confidence to 0 and leave fields as empty strings.`,
    },
  ];
  try {
...
// ── Phase 2: classify + rename a single file ──────────────────────────────────
...
{unit} = bare unit number (11, not #11 or Apt 11). Keep original extension if not PDF.
...
{ "category": "...", "renamed_to": "...", "unit": "...", "building_address": "...", "confidence": 0.0-1.0 }`,
  ...
  try {
    ...
    const bSlug = p.building_address
      ? slugify(p.building_address.split(",")[0].trim()) || dealSlug
      : dealSlug;
    return {
  ...
  try {
    ...
    if (action === "detect_address") {
      ...
      for (const [k, v] of formData.entries()) {
        if (k === "files" && v instanceof File) { files.push(v); break; } // first file only
      }
      if (!files.length) {
      ...
      const result = await detectAddress(files[0]);
      return new Response(JSON.stringify(result), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // ── Phase 2: full processing (sequential to stay within memory) ───────────
    if (action === "process") {
      ...
      // ── Process ONE file at a time to stay within memory limits ──────────────
      for (const file of files) {
        ...
        await supabase.storage.from("dd-documents").upload(storagePath, file, { upsert: true });
